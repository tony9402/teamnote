...
int main() {
    ...
    while(~--T) {
        ...
        for(auto &[y, x] : points) cin >> y >> x;
        auto tmp = points;
        sort(points.begin(), points.end(), [&](const pair<ll, ll> &a, const pair<ll, ll> &b) { return a.second != b.second ? a.second < b.second : a.first < b.first; });
        points.erase(unique(points.begin(), points.end()), points.end());
        KDTree<ll> seg(points);
        seg.build();
        for(const auto &it: tmp) cout << seg.query(it) << '\n';
    }
    return 0;
}